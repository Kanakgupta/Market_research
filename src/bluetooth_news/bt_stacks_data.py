...
def _stack(slug, name, vendor, category, license, spec, tagline, overview,
           ...
           chips="", footprint="", commercial="", strengths=None, weaknesses=None):
    return {
        ...
        "links": links or [],
        "chips": chips, "footprint": footprint, "commercial": commercial,
...
BT_STACKS = [
    ...
    _stack(
        ...
        chips="AIROC CYW20829 (BLE MCU, Bluetooth 5.4 + Channel Sounding), CYW20819/20820 (dual-mode BT/BLE), CYW5557x (Wi-Fi 6E/7 + Bluetooth combo), CYW43439/43012 (Wi-Fi 4 + BT combo), CYW89820 (automotive BT), PSoC 62/63 + combo designs, plus AIROC CYBT-/CYW9-series certified modules.",
        footprint="Host + controller shipped together in ModusToolbox. Runs on Arm Cortex-M (CYW20829 M33, PSoC 6 M4/M0+) and as an HCI controller under Linux/RTOS hosts; FreeRTOS + ThreadX supported. On-chip stack fits typical BLE-MCU budgets (order of hundreds of KB flash).",
        commercial="Royalty-free stack bundled with AIROC silicon via ModusToolbox; direct Infineon FAE/engineering support, long-term automotive & industrial lifecycle, and Bluetooth SIG qualification assistance. Mature CYW/PSoC + ModusToolbox ecosystem.",
        strengths=["Silicon + stack co-designed and power-tuned on Infineon radios",
                   "One vendor for Wi-Fi+BT combo, BLE MCU and automotive BT with long lifecycle support",
                   "CYW20829 delivers Bluetooth 5.4 + Channel Sounding for ranging / digital-key",
                   "Turnkey qualification + ModusToolbox tooling shorten time-to-market"],
        weaknesses=["Locked to Infineon silicon -- no cross-vendor portability layer",
                    "Newest LE Audio profiles (HAP/GMAP) trail audio-specialist stacks on some parts",
                    "Smaller community/tutorial base than Nordic"],
    ),
    _stack(
        ...
        chips="nRF52 series (nRF52840/52833/52811/52832), nRF53 dual-core (nRF5340), nRF54 series (nRF54L15 / nRF54H20 -- Bluetooth 6.0 incl. Channel Sounding), nRF21540 FEM for range; nRF7002 companion IC for Wi-Fi.",
        footprint="LE-only. SoftDevice = precompiled controller binary (S112/S113/S132/S140) on nRF52; nRF Connect SDK (Zephyr) with SoftDevice Controller on nRF53/54. Cortex-M; hundreds of KB flash; Zephyr RTOS.",
        commercial="Free SDK + royalty-free SoftDevice; excellent docs, DevAcademy and DevZone community; commercial support via Nordic/distributors. Very mature developer ecosystem.",
        strengths=["Best-in-class documentation, tooling (nRF Connect) and community",
                   "Early to new LE features -- LE Audio and Channel Sounding on nRF54",
                   "Portable path via Zephyr (nRF Connect SDK)"],
        weaknesses=["No Classic BR/EDR -- cannot do A2DP/HFP/SPP legacy accessories",
                    "Wi-Fi is a separate companion IC (nRF7002), not integrated combo silicon",
                    "Binary SoftDevice model less flexible than full-source stacks"],
    ),
    _stack(
        ...
        chips="EFR32BG22/BG24/BG27 (Series 2 Bluetooth SoCs), EFR32MG24 multiprotocol (BT + Matter/Thread/Zigbee), BGM/MGM certified modules; xG24-class parts add LE Audio + Channel Sounding.",
        footprint="LE + Bluetooth Mesh; Cortex-M33; Simplicity SDK; bare-metal or RTOS; Secure Vault hardware security. Multiprotocol time-slices Bluetooth with Zigbee/Thread/Matter on one radio.",
        commercial="Free Simplicity Studio/SDK; direct Silicon Labs support; strong smart-home / Matter ecosystem with reference designs.",
        strengths=["Leading Bluetooth Mesh + multiprotocol (BT + Matter/Thread/Zigbee) on one chip",
                   "Secure Vault hardware security for connected-product compliance",
                   "Good LE Audio + Channel Sounding on xG24"],
        weaknesses=["LE-only -- no Classic audio/serial",
                    "Audio-profile breadth narrower than audio-specialist stacks",
                    "Ecosystem centered on smart-home/industrial, less on phones/audio"],
    ),
    _stack(
        ...
        chips="SimpleLink CC2340 (CC23xx, low-cost BLE 5.4), CC2642/CC2652 (CC26xx multiprotocol BT + 802.15.4), CC1352 (sub-GHz + BLE), CC2745/CC27xx (adds Channel Sounding).",
        footprint="LE-only; Cortex-M; TI-RTOS/FreeRTOS/no-RTOS; very low power (autonomous sensor-controller engine). Host + controller in SimpleLink SDK.",
        commercial="Free SimpleLink SDK; TI E2E support; long industrial lifecycle and supply; strong in metering/industrial/medical.",
        strengths=["Excellent low power and multiprotocol (BLE + 802.15.4/Thread/Matter)",
                   "CC27xx adds Channel Sounding for secure ranging",
                   "Long industrial lifecycle and supply assurance"],
        weaknesses=["Not an LE Audio audio stack (no BAP/CAP out of box)",
                    "No Classic BR/EDR",
                    "Bluetooth Mesh is not a core focus"],
    ),
    _stack(
        ...
        chips="KW45/KW47 & KW38 (BLE MCUs), K32W148 (BT + 802.15.4/Matter), RW612/RW610 (Wi-Fi 6 + BT + 802.15.4 tri-radio), 88W combos, i.MX RT / i.MX 8/9 (BlueZ host).",
        footprint="LE on KW MCUs; dual-mode on combo/i.MX; Cortex-M and Cortex-A; FreeRTOS/Zephyr/Linux. Capability varies widely across families.",
        commercial="Free MCUXpresso SDK; NXP support with long automotive/industrial lifecycle; broad portfolio under one vendor.",
        strengths=["Widest span: MCU + tri-radio combo + Linux app-processor BT under one vendor",
                   "Strong automotive & industrial lifecycle",
                   "LE Audio on newer connectivity silicon"],
        weaknesses=["Very family-dependent -- no single unified stack story",
                    "Channel Sounding availability part-dependent",
                    "Combo/i.MX BT is a different stack than KW-MCU BT"],
    ),
    _stack(
        ...
        chips="STM32WB55/WB35/WB15 (dual-core M4 + M0 radio), STM32WBA52/WBA55/WBA6x (single-core M33, BLE 5.4 + LE Audio), BlueNRG-2 / BlueNRG-LP standalone network processors.",
        footprint="LE-only; Cortex-M; open STM32Cube middleware over a proprietary controller; FreeRTOS/ThreadX/bare-metal; concurrent 802.15.4/Thread/Matter/Zigbee on WB/WBA.",
        commercial="Free STM32Cube; huge STM32 ecosystem and distribution; ST support; cost-competitive with second-source availability.",
        strengths=["Massive STM32 ecosystem, tooling and second-source availability",
                   "Cost-effective LE + concurrent Thread/Matter/Zigbee",
                   "WBA family adds LE Audio"],
        weaknesses=["LE-only -- no Classic",
                    "No Channel Sounding in current WB/WBA",
                    "LE Audio only on WBA, not the older WB line"],
    ),
    _stack(
        ...
        chips="ESP32 (classic dual-mode BT + Wi-Fi), ESP32-C3/C6/C2/H2 (LE-only; C6/H2 add 802.15.4/Thread/Matter/Zigbee), ESP32-S3 (BLE), newer ESP32-C5 (Wi-Fi 6 + BLE).",
        footprint="Xtensa / RISC-V cores; FreeRTOS (ESP-IDF); selectable host -- Bluedroid (dual-mode) or Apache NimBLE (LE, smaller). Wi-Fi/BT coexistence built in.",
        commercial="Apache-2.0 SDK, very low silicon cost, enormous maker/OEM community; commercial support via Espressif.",
        strengths=["Cheapest path to Wi-Fi + BT on one SoC with a huge community",
                   "Choice of Bluedroid (dual-mode) or NimBLE (LE) host",
                   "Built-in Wi-Fi/BT coexistence"],
        weaknesses=["Classic audio only on original ESP32; C-series are LE-only",
                    "LE Audio profile stack still maturing",
                    "No Channel Sounding; higher power than dedicated BLE MCUs"],
    ),
    _stack(
        ...
        chips="QCC30xx/QCC51xx/QCC3xxx earbud & headset SoCs, FastConnect combos (e.g. FastConnect 7800), and Snapdragon mobile/compute BT subsystems.",
        footprint="Dual-mode with premium audio DSP; runs on Qualcomm's own cores/RTOS in audio SoCs and within Snapdragon. Not offered as a portable MCU stack.",
        commercial="Licensed to product makers; premium audio IP (aptX, Snapdragon Sound); Qualcomm engineering support; ecosystem tied to Qualcomm silicon.",
        strengths=["Premium audio leadership -- aptX Lossless/Adaptive, Snapdragon Sound",
                   "Early LE Audio + Auracast, plus GMAP gaming audio",
                   "Deep Android/Snapdragon integration"],
        weaknesses=["Best value only when both endpoints are Qualcomm",
                    "Closed ecosystem; not a general-purpose MCU stack",
                    "Premium cost tier"],
    ),
    _stack(
        ...
        chips="RTL8761/RTL8762 (BLE SoCs), RTL8763/RTL8773 (audio SoCs for earbuds), RTL8852/RTL8822 (Wi-Fi + BT combos for PC/TV/IoT).",
        footprint="Dual-mode; combo integration for PC/TV; vendor RTOS/SDK; dedicated audio SoCs for earbuds. Documentation lighter than tier-1 vendors.",
        commercial="Cost-driven licensing bundled with silicon; very high volume in PC/TV/combo; support via Realtek/distributors.",
        strengths=["Very high-volume, low-cost combo and audio silicon",
                   "Solid mainstream dual-mode for PC/TV/earbud",
                   "Broad market availability"],
        weaknesses=["Newest LE features lag the leaders",
                    "Lighter docs and developer support",
                    "Less suited to leading-edge LE Audio / ranging"],
    ),
    _stack(
        ...
        chips="WBZ451/WBZ351 (PIC32CX-BZ, BLE + 802.15.4/Zigbee), RN4870/RN4871 modules, BM70/BM71, IS187x, and former Atmel ATBTLC1000.",
        footprint="LE (Classic on combo parts); Cortex-M / PIC32; MPLAB Harmony / BluSDK; BLE + Zigbee concurrency on WBZ.",
        commercial="Free MPLAB Harmony; Microchip support with very long lifecycle; strong pull-through for existing PIC/AVR customers.",
        strengths=["Fits existing Microchip/PIC/AVR designs and toolchain",
                   "Long product lifecycle and supply",
                   "BLE + Zigbee concurrency on WBZ"],
        weaknesses=["Generally no LE Audio / BT 6.0 on mainstream parts",
                    "Feature timing trails the leaders",
                    "Smaller BLE developer mindshare"],
    ),
    _stack(
        ...
        chips="DA14531 (SmartBond TINY, ultra-low-cost BLE), DA1469x (SmartBond, Cortex-M33), DA1470x (adds LE Audio), plus SmartBond modules.",
        footprint="LE-only; Cortex-M0+/M33; SmartBond SDK / FreeRTOS; famous ultra-low-power; small flash/RAM options incl. the tiny DA14531.",
        commercial="Free SmartBond SDK; Renesas support and lifecycle; strong in wearables, HID and beacons.",
        strengths=["Class-leading low power for wearables/HID/beacons",
                   "DA14531 is extremely small and low-cost",
                   "LE Audio on DA1470x-class parts"],
        weaknesses=["LE-only, no Classic",
                    "No Channel Sounding on mainstream parts",
                    "Bluetooth Mesh is not a focus"],
    ),
    ...
    _stack(
        ...
        chips="Silicon-independent -- runs over any qualified HCI controller (customer's chosen radio). Widely deployed on automotive infotainment SoCs (Qualcomm/NXP/Renesas application processors).",
        footprint="Host stack (bring your own controller); portable across Linux/RTOS and CPU architectures; ANSI-C, configurable small footprint; automotive processes aligned to ISO 26262.",
        commercial="Commercial source license + profile packages; automotive-grade support, long maintenance and a qualification program. Used by many car OEMs and Tier-1s.",
        strengths=["Portable, chip-independent dual-mode with the full Classic profile set (HFP/PBAP/MAP/A2DP/AVRCP)",
                   "Automotive-grade support, maintenance and functional-safety processes",
                   "Reuse across silicon generations without rewriting the stack"],
        weaknesses=["License cost; you integrate onto your own silicon (no chip included)",
                    "Bluetooth Mesh is not a focus",
                    "Channel Sounding depends on the chosen controller"],
    ),
    _stack(
        ...
        chips="Silicon-independent IP; embedded (often re-branded) inside many silicon-vendor SDKs and runs over their controllers.",
        footprint="Portable host + controller C IP; small configurable footprint; RTOS/bare-metal/Linux; LE Audio available.",
        commercial="Licensed IP (Mindtree / LTTS) with integration and qualification services; the hidden engine behind several vendor stacks.",
        strengths=["Proven, fully-featured dual-mode + LE Audio, widely field-deployed",
                   "Vendors embed and re-brand it -- mature and interoperable",
                   "Full Classic + LE profile coverage"],
        weaknesses=["Delivered as IP -- integration effort on you or the vendor",
                    "Not a chip; qualification happens under the integrator",
                    "Channel Sounding is controller-dependent"],
    ),
    _stack(
        ...
        chips="Silicon-independent source stack; integrated onto the customer's own MCU/SoC plus a qualified controller.",
        footprint="Portable dual-mode C source; RTOS/bare-metal; customizable small footprint; profiles as add-on packages.",
        commercial="Commercial source license + integration services; positioned for embedded and automotive customization.",
        strengths=["Tailorable source with a strong Classic + LE profile set",
                   "Integration and customization services",
                   "Portable across silicon"],
        weaknesses=["Smaller ecosystem than the largest licensable stacks",
                    "LE Audio / Channel Sounding config- and controller-dependent",
                    "You supply the qualified radio"],
    ),
    _stack(
        ...
        chips="LE stack + controller IP licensed into SoCs/ASICs; appears inside the integrating vendor's silicon.",
        footprint="Compact LE host/controller IP for constrained designs; RTOS/bare-metal; sized for cost/area-sensitive chips.",
        commercial="IP licensing to semiconductor makers; qualification at the integrator; ingredient technology, not an end product.",
        strengths=["Compact licensable LE IP for chip/module makers",
                   "Good fit for area/cost-sensitive designs",
                   "Vendor-neutral ingredient IP"],
        weaknesses=["LE-only IP; no Classic",
                    "Not off-the-shelf -- for integrators only",
                    "Feature set depends on the licensed configuration"],
    ),
    ...
    _stack(
        ...
        chips="Runs on dozens of SoCs: Nordic nRF52/53/54, NXP KW, STM32WB/WBA, TI CC13/26xx, Espressif ESP32, Silicon Labs EFR32, plus HCI over any external controller.",
        footprint="Open host (+ optional controller); Zephyr RTOS; Cortex-M / RISC-V; highly configurable via Kconfig from tiny to feature-rich builds; LE-only.",
        commercial="Apache-2.0, no license fee; support via vendors (e.g. Nordic nRF Connect SDK) or integrators/consultancies; very active upstream.",
        strengths=["Cross-vendor portability -- one codebase across many MCUs",
                   "Fast-moving upstream: LE Audio and Channel Sounding landing",
                   "No per-unit royalty; deep customization"],
        weaknesses=["No Classic BR/EDR profiles (LE-only)",
                    "You own integration, qualification and maintenance (no single SLA)",
                    "Feature maturity varies by SoC/driver"],
    ),
    _stack(
        ...
        chips="Any Linux host with a standard HCI controller (USB/UART/SDIO dongles and SoCs); common on Raspberry Pi, i.MX and Qualcomm/Rockchip SBCs.",
        footprint="Linux host stack with D-Bus API; needs a Linux-class CPU and MB-scale RAM; not for MCU-only endpoints; dual-mode.",
        commercial="GPL/LGPL open source; support via distros/integrators; qualification at the product level.",
        strengths=["The default, ubiquitous Linux dual-mode stack with broad profiles + interop",
                   "D-Bus APIs and a huge deployed base on gateways/edge",
                   "Runs over any HCI controller"],
        weaknesses=["Requires a Linux-class host -- not for tiny MCUs",
                    "LE Audio support is comparatively recent/evolving",
                    "No Channel Sounding yet"],
    ),
    _stack(
        ...
        chips="MCU-class parts: Nordic nRF5x, NXP, Dialog, and bundled on Espressif ESP32-C/S/H; runs over its own or vendor controllers.",
        footprint="LE-only host (+ optional controller); very small footprint; Mynewt/FreeRTOS/Zephyr ports; Apache-2.0.",
        commercial="Apache-2.0, no fee; community + integrator support; qualification per product.",
        strengths=["Very small footprint and permissive license",
                   "Portable across MCUs; bundled in ESP-IDF",
                   "Good for constrained LE endpoints"],
        weaknesses=["LE-only; no Classic",
                    "No LE Audio profile stack (no BAP/CAP)",
                    "No Channel Sounding; DIY qualification/maintenance"],
    ),
    _stack(
        ...
        chips="Broad: Raspberry Pi Pico W (CYW43439), Nordic, TI, Espressif, Realtek, plus POSIX/USB dongles; runs over many controllers.",
        footprint="Portable dual-mode host; runs from tiny MCUs to Linux; static memory (no malloc); wide RTOS/bare-metal support.",
        commercial="Free for non-commercial use; commercial license from BlueKitchen with support and qualification help.",
        strengths=["Clean, portable dual-mode with surprising profile breadth for its size",
                   "Runs from tiny MCUs to Pi Pico W to Linux",
                   "Great for custom/research firmware"],
        weaknesses=["Needs stronger in-house BT expertise to productionize",
                    "Smaller ecosystem than vendor stacks",
                    "No Channel Sounding"],
    ),
    _stack(
        ...
        chips="Runs on a PC/host driving real controllers over HCI (USB/UART), or fully virtual with no hardware.",
        footprint="Python host stack -- for test/virtualization, not embedded firmware; needs a Python host.",
        commercial="Apache-2.0; Google-maintained; a testing/tooling stack, not a product stack.",
        strengths=["Excellent for test automation, virtual peers and interop debugging",
                   "Scriptable in Python; drives real or virtual controllers",
                   "Free and open"],
        weaknesses=["Not for shipping product firmware",
                    "Runs in Python -- not for constrained targets",
    ...
    _stack(
        ...
        chips="Runs on the phone/tablet SoC's BT subsystem (Qualcomm/MediaTek/Samsung/Google Tensor) via HCI; billions of devices.",
        footprint="Android platform host stack (Fluoride -> Gabeldorsche); Linux/Android; large; deeply integrated with the framework.",
        commercial="AOSP open source; validated per handset by the OEM; the phone-side interop target.",
        strengths=["Billions of devices -- the accessory interop baseline on Android",
                   "Dual-mode incl. LE Audio + ASHA hearing aids",
                   "Deep framework integration"],
        weaknesses=["A phone OS stack -- not a drop-in MCU peripheral stack",
                    "Capabilities vary by OEM and Android version",
                    "No standardized Channel Sounding exposure yet"],
    ),
    _stack(
        ...
        chips="Runs on Apple-silicon BT subsystems in iPhone/iPad/Mac/Watch; exposed via CoreBluetooth (LE) plus higher-level frameworks.",
        footprint="Platform host stack; iOS/macOS; deeply integrated; third-party access is API-gated (no raw Classic SPP).",
        commercial="Proprietary/platform; validated per device; MFi program for accessories; the other phone-side interop target.",
        strengths=["The iOS/macOS interop baseline for accessory makers",
                   "Dual-mode incl. LE Audio; polished pairing (AirPods/Find My)",
                   "Consistent behavior across Apple devices"],
        weaknesses=["No third-party Classic SPP (uses MFi/iAP or LE)",
                    "Many features gated behind Apple frameworks (ANCS/Find My/MFi)",
                    "No third-party Channel Sounding exposure"],
    ),
    _stack(
        ...
        chips="Runs on PCs with standard HCI controllers (Intel AX2xx, Realtek, MediaTek combos); exposed via WinRT APIs.",
        footprint="Windows platform host stack; Windows on x86/Arm; needs a PC-class host; dual-mode.",
        commercial="Proprietary/platform (in-box); validated per PC/platform; the PC-side interop target.",
        strengths=["The Windows PC interop baseline with broad Classic accessory support",
                   "WinRT GATT/RFCOMM/advertising APIs for app developers",
                   "Runs over standard controllers"],
        weaknesses=["LE Audio / Auracast still maturing on Windows",
                    "PBAP/MAP not first-class in platform APIs",
                    "No Channel Sounding exposure"],
    ),
]
